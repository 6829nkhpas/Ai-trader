...
// de-duplicated by `aggregateCandles`). The pure paint-classification helpers
// (`classifyRealtimePaint` / `isViewAtRightEdge` / `shouldFollowRightEdge`)
// live in `charting/realtimePaint` so they can be unit-tested without the DOM;
// this hook just wires their decisions into the lightweight-charts series.
