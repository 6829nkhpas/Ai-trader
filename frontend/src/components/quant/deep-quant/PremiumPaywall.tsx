...
export default function PremiumPaywall({ onUpgradeClick }: PremiumPaywallProps) {
  ...
  return (
    <div className="flex-grow flex flex-col items-center justify-center gap-6 p-6 w-full h-full min-h-[450px] max-w-md mx-auto text-center animate-in fade-in duration-300">
      {/* Large SVG Illustration */}
      <div className="w-72 h-40 flex items-center justify-center shrink-0">
      ...
      <div className="w-full max-w-sm flex flex-col gap-2 pt-1">
        <button
          ...
          disabled={upgrading}
          onClick={handleClick}
          className="w-full flex items-center justify-center gap-2 rounded-none bg-text-primary text-surface hover:bg-text-secondary px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all active:scale-[0.98] disabled:opacity-50 border border-text-primary"
        >
          {upgrading ? (
            <>
