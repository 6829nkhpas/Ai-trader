...
export default function TradeQaPanel() {
  ...
  return (
    <div className="flex flex-col font-mono bg-black/85 border border-slate-800 rounded-xl overflow-hidden shadow-2xl mt-3">
      {/* Header */}
      ...
      <div className="max-h-[320px] min-h-[80px] overflow-y-auto p-3 space-y-3 scrollbar-thin scrollbar-track-slate-950/20 scrollbar-thumb-slate-800">
        {qaMessages.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 py-6 text-center select-none">
            <MessageCircle size={18} className="text-teal-400/50" />
          ...
          qaMessages.map((msg) =>
            msg.role === 'user' ? (
              <div key={msg.id} className="flex justify-end animate-fade-in font-sans">
                <div className="max-w-[85%] bg-teal-600/20 text-slate-100 border border-teal-500/25 rounded-xl rounded-br-sm px-3 py-2 text-[11px] leading-relaxed shadow-sm">
                  <div className="flex items-center gap-1.5 text-[9px] text-teal-300 font-bold uppercase tracking-wider mb-1 select-none">
                    <User size={10} />
                    You
                  </div>
              ...
              <div key={msg.id} className="flex justify-start animate-fade-in font-sans w-full">
                <div
                  className={`max-w-[95%] w-full rounded-xl rounded-bl-sm px-3 py-2 text-[11px] leading-relaxed shadow-sm ${
                    msg.error
                      ? 'bg-rose-950/30 text-rose-200 border border-rose-500/25'
                      : 'bg-slate-900/40 text-slate-100 border border-slate-800/40'
                  }`}
                >
                  <div
                    className={`flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider mb-1 select-none ${
                      msg.error ? 'text-rose-400' : 'text-emerald-400'
                    }`}
                  >
                  ...
                  {msg.activity && msg.activity.length > 0 && (
                    <div className="mb-1.5 flex flex-col gap-0.5">
                      {msg.activity.map((line, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-1.5 text-[8.5px] font-mono text-teal-400/70"
                        >
      ...
      {/* Composer */}
      <div className="shrink-0 border-t border-slate-800 bg-slate-950/70 p-2.5">
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="flex-1 resize-none rounded-lg bg-black/50 border border-slate-800 px-3 py-2 text-[11px] font-sans text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-teal-500/40 focus:ring-1 focus:ring-teal-500/20 disabled:opacity-50 disabled:cursor-not-allowed scrollbar-thin"
          />
          <button
            type="button"
            onClick={handleSend}
            disabled={!canSend}
            title="Send question"
            className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[11px] font-bold uppercase tracking-wider transition-all duration-200 ${
              canSend
                ? 'bg-gradient-to-r from-teal-600 to-cyan-600 text-white border border-teal-500/40 hover:from-teal-500 hover:to-cyan-500 hover:shadow-lg hover:shadow-teal-500/20 active:scale-[0.98]'
