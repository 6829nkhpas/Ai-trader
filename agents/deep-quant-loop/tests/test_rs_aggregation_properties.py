"""Property-based test for per-relative-strength journal aggregation
(journal.py, task 11.3).

Feature: relative-strength-context

This module implements design **Property 26: Per-relative-strength aggregation
reports correct win-rate and expectancy**:

    Grouping scored (win/loss) trades by the relative-strength-extended
    ``setup_key`` reports, for each group, a win-rate equal to the fraction of
    that group's scored trades that are wins (a value in [0.0, 1.0]) and an
    expectancy equal to the mean R-multiple of that group's scored trades;
    groups holding fewer scored trades than ``LOW_SAMPLE_THRESHOLD`` are flagged
    as a weak prior.

Validates: Requirements 10.4, 10.5.

The implementation under test lives in ``journal.py``:
  - ``derive_setup_tags`` / ``setup_key_from_tags`` — build the
    relative-strength-extended ``setup_key`` (the ``rs:<value>`` dimension is
    appended at a fixed position by ``_relative_strength_tag``).
...
``test_regime_aggregation_properties.py`` exactly.
"""
...
_fd, _TMP_DB = tempfile.mkstemp(prefix="rs_agg_journal_", suffix=".db")
os.close(_fd)
...
# ── Strategies ───────────────────────────────────────────────────────────────
# The eight fixed relative-strength tag values (R10.3) each mapped to a
# relative-strength defensibility entry that ``journal._relative_strength_tag``
# collapses back to exactly that tag value.
_RS_VALUE_TO_ENTRY = {
    "leader-aligned": {"relative_strength_state": "leader", "alignment": "aligned", "available": True},
    "leader-misaligned": {"relative_strength_state": "leader", "alignment": "misaligned", "available": True},
    "laggard-aligned": {"relative_strength_state": "laggard", "alignment": "aligned", "available": True},
    "laggard-misaligned": {"relative_strength_state": "laggard", "alignment": "misaligned", "available": True},
    "inline-neutral": {"relative_strength_state": "inline", "alignment": "neutral", "available": True},
    # Residual inline x directional pairings collapse to the bare Alignment.
    "aligned": {"relative_strength_state": "inline", "alignment": "aligned", "available": True},
    "misaligned": {"relative_strength_state": "inline", "alignment": "misaligned", "available": True},
    "unknown": {"available": False},
}

_rs_value = st.sampled_from(sorted(_RS_VALUE_TO_ENTRY.keys()))
...
def _make_decision(rs_value, direction):
    """Build a committed-decision dict whose relative strength collapses to
    ``rs_value``."""
    return {
        "action": direction,
        "defensibility": {"relative_strength": dict(_RS_VALUE_TO_ENTRY[rs_value])},
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# Feature: relative-strength-context, Property 26: Per-relative-strength
# aggregation reports correct win-rate and expectancy
# ─────────────────────────────────────────────────────────────────────────────
...
def test_property_26_per_relative_strength_aggregation_metrics(specs, threshold):
    """Validates: Requirements 10.4, 10.5

    Seed scored (and some excluded) trades grouped under relative-strength-
    extended ``setup_key``s, then run the real aggregation and assert per group:
      * win-rate == wins / scored and lies in [0.0, 1.0]   (R10.4)
      * expectancy == mean R-multiple of the group's scored trades  (R10.4)
      * a group with fewer scored trades than LOW_SAMPLE_THRESHOLD is a weak
        prior (its trades_scored < surfaced low_sample_threshold)    (R10.5)
    """
    ...
    expected: dict = {}
    for spec in specs:
        decision = _make_decision(spec["rs_value"], spec["direction"])
    ...
    # Every seeded relative-strength-extended setup_key is reported exactly once.
    ...
    for key, exp in expected.items():
        ...
        # ── Win-rate: fraction of scored trades that are wins, in [0, 1] (R10.4)
        if scored == 0:
        ...
        # ── Weak-prior flagging (R10.5): a group with fewer scored trades than
