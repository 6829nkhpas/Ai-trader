...
def test_property_24_debate_tag_is_fixed_bounded_deterministic(decision):
    ...
    #    (R9.1). The options ``opt:``, opportunity ``tier:`` and event ``evt:``
    #    dimensions are appended after it, so ``db:`` is no longer the final tag. ─
    ...
    #    the final tag is the event ``evt:`` dimension (the opportunity ``tier:``
    #    dimension is now the second-to-last tag). ──────────────────────────────
    ...
    assert tags[-2].startswith("tier:"), f"tier: tag must be the second-to-last tag, got tags={tags}"
