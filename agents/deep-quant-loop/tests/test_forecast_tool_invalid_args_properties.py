"""Property-based test for forecast invalid-argument rejection (tools.py, task 6.5).

Feature: volatility-aware-forecaster

This Hypothesis property exercises **Property 15: The tool rejects invalid
arguments without raising**. The ``get_forecast`` tool validates its arguments
...
property is fully self-contained: it requires no live Rust Tool_Server or network
access. As an extra guard the test patches ``tools.httpx.post`` to fail loudly if
it is ever reached, proving the validation short-circuits before any network
call. The generators below deliberately produce ONLY invalid combinations —
empty/whitespace symbols (which short-circuit on the symbol check) and/or
unsupported timeframe strings (which short-circuit on the timeframe check) — and
never a valid-symbol + valid-timeframe pair that would trigger a candle fetch.

The sys.path / import pattern and the ``_raw`` @tool-unwrap helper mirror
``tests/test_rs_tool_invalid_args_properties.py``.
"""
...
def _no_network(*args, **kwargs):
    ...
    raise AssertionError(
        "get_forecast performed a network fetch on invalid arguments; "
...
# timeframe) pairs are rejected irrespective of the other argument.
_PROPOSED_DIRECTION = st.sampled_from(["", "BUY", "SELL", "HOLD", "buy", "garbage"])


@st.composite
def _invalid_args(draw):
    """Draw a (symbol, timeframe, proposed_direction) tuple whose (symbol,
    timeframe) pair is guaranteed invalid.
...
# ─────────────────────────────────────────────────────────────────────────────
# Property 15: invalid arguments -> structured error, never raises
# ─────────────────────────────────────────────────────────────────────────────

# Feature: volatility-aware-forecaster, Property 15: The tool rejects invalid arguments without raising
@settings(max_examples=100, deadline=None)
@given(args=_invalid_args())
def test_property_15_forecast_tool_rejects_invalid_args_without_raising(args):
    """Feature: volatility-aware-forecaster, Property 15: The tool rejects
    invalid arguments without raising.

    For any whitespace-only/empty symbol, or any timeframe not in the supported
    set, ``get_forecast`` returns a structured error result (a dict with an
