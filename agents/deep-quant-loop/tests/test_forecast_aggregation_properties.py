"""Property-based test for per-forecast journal aggregation
(journal.py, task 12.5).

Feature: volatility-aware-forecaster

This module implements design **Property 30: Per-forecast aggregation reports
correct win-rate and expectancy**:

    Grouping scored (win/loss) trades by the forecast-extended ``setup_key``
...
The implementation under test lives in ``journal.py``:
  - ``derive_setup_tags`` / ``setup_key_from_tags`` — build the forecast-extended
    ``setup_key`` (the ``fc:<value>`` dimension is appended at a fixed position
    by ``_forecast_tag``, collapsing Forecast_Alignment x Up_Probability band).
...
_fd, _TMP_DB = tempfile.mkstemp(prefix="fc_agg_journal_", suffix=".db")
os.close(_fd)
...
# ── Strategies ───────────────────────────────────────────────────────────────
# The seven fixed forecast tag values (R11.3) each mapped to a forecast
# defensibility entry that ``journal._forecast_tag`` collapses back to exactly
# that tag value. The confidence band is ``strong`` when
# ``abs(up_probability - 0.5) >= FC_STRONG_PROB_SPLIT`` (0.15), else ``weak``:
#   * strong -> up_probability 0.85 (abs 0.35 >= 0.15) for 'up'-leaning entries
#   * weak   -> up_probability 0.55 (abs 0.05 <  0.15)
_FC_VALUE_TO_ENTRY = {
    "aligned-strong": {"forecast_alignment": "aligned", "up_probability": 0.85, "available": True},
    "aligned-weak": {"forecast_alignment": "aligned", "up_probability": 0.55, "available": True},
    "misaligned-strong": {"forecast_alignment": "misaligned", "up_probability": 0.15, "available": True},
    "misaligned-weak": {"forecast_alignment": "misaligned", "up_probability": 0.45, "available": True},
    "neutral-strong": {"forecast_alignment": "neutral", "up_probability": 0.80, "available": True},
    "neutral-weak": {"forecast_alignment": "neutral", "up_probability": 0.50, "available": True},
    "unknown": {"available": False},
}

_fc_value = st.sampled_from(sorted(_FC_VALUE_TO_ENTRY.keys()))
...
def _make_decision(fc_value, direction):
    """Build a committed-decision dict whose forecast collapses to ``fc_value``."""
    return {
        "action": direction,
        "defensibility": {"forecast": dict(_FC_VALUE_TO_ENTRY[fc_value])},
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# Feature: volatility-aware-forecaster, Property 30: Per-forecast aggregation
...
def test_property_30_per_forecast_aggregation_metrics(specs, threshold):
    """Validates: Requirements 11.5

    Seed scored (and some excluded) trades grouped under forecast-extended
    ``setup_key``s, then run the real aggregation and assert per group:
      * win-rate == wins / scored and lies in [0.0, 1.0]            (R11.5)
      * expectancy == mean R-multiple of the group's scored trades  (R11.5)
      * a group with fewer scored trades than LOW_SAMPLE_THRESHOLD is a weak
        prior (its trades_scored < surfaced low_sample_threshold)    (R11.5)
    """
    ...
    expected: dict = {}
    for spec in specs:
        decision = _make_decision(spec["fc_value"], spec["direction"])
    ...
    # Every seeded forecast-extended setup_key is reported exactly once.
    ...
    for key, exp in expected.items():
        ...
        # ── Win-rate: fraction of scored trades that are wins, in [0, 1] (R11.5)
        if scored == 0:
        ...
        # ── Weak-prior flagging (R11.5): a group with fewer scored trades than
