"""Unit tests for the `get_order_flow` tool shape (task 6.3).

Feature: order-flow-context

Requirement 5.1:
    THE Order_Flow_Tool SHALL be exposed to the Deep_Quant_Agent as an
    `@tool`-decorated function named `get_order_flow` following the existing tool
    pattern in `tools.py`.

Requirement 5.2:
    THE Order_Flow_Tool SHALL accept a `symbol` argument, a `timeframe`
...
function as a ``StructuredTool``, so we assert against that wrapper:
  ...
  (a) `get_order_flow` is a StructuredTool / BaseTool instance (i.e.
      `@tool`-decorated), not a bare function;
  (b) its `.name` is exactly ``"get_order_flow"``;
...
def test_get_order_flow_is_tool_decorated():
    ...
    """
    assert isinstance(get_order_flow, BaseTool)
    assert isinstance(get_order_flow, StructuredTool)
    assert isinstance(tools.get_order_flow, BaseTool)


def test_get_order_flow_name():
    """Validates: Requirements 5.1

    The exposed tool name must be exactly ``get_order_flow`` so the agent and
    the registry can address it.
    """
    assert get_order_flow.name == "get_order_flow"


# ── Requirement 5.2: accepts `symbol`, `timeframe`, optional direction ───────
def test_get_order_flow_accepts_symbol_timeframe_direction():
...
def test_get_order_flow_proposed_direction_is_optional():
    """Validates: Requirements 5.2
