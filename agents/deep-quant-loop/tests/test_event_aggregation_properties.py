# Feature: earnings-event-risk-gate, Property 25: Per-event-risk aggregation reports correct win-rate and expectancy
"""Property-based test for per-event-risk journal aggregation (journal.py, task 7.3).

Feature: earnings-event-risk-gate

This module implements design **Property 25: Per-event-risk aggregation reports
correct win-rate and expectancy**:

    For any set of recorded/scored trades, grouping scored (win or loss) trades
    by the event-extended ``setup_key`` yields, for each group, a win-rate equal
    to the fraction of scored trades that are wins (within ``[0.0, 1.0]``) and an
...
varying defensibility dimension is the event entry (the Event_Risk
classification produced by the Earnings/Event-Date Risk Gate), so each trade's
``setup_key`` differs only by its ``evt:`` tag and trades sharing an event-risk
bucket aggregate together. Each trade is recorded via
``journal.record_backtest_trade`` with a win / loss / expired status and (for
win/loss) a finite R-multiple. The test then calls ``journal.get_stats`` and
asserts that, for every per-setup (``by_setup``) group, the reported
``win_rate`` and ``expectancy_r`` equal the independently computed expected
values and the ``low_sample`` flag matches the scored count against the
...
The sys.path / import pattern mirrors the sibling ``test_*_aggregation_*``
modules, and the shared ``tests/conftest.py`` journal-global isolation is left
undisturbed.
"""
...
# untouched.
_TMP_DB = os.path.join(tempfile.mkdtemp(prefix="evt_agg_journal_"), "trade_journal.db")
...
# The three categorical Event_Risk classifications, plus a sentinel that drives
# the ``evt:unknown`` bucket via an explicitly-unavailable event entry. Each maps
# to a distinct fixed ``evt:`` tag, so trades sharing an Event_Risk aggregate
# into the same event-extended setup_key group.
_EVENT_RISKS = ["clear", "imminent", "through_event", "__unavailable__"]

# Already-scored statuses only (no 'open' -> no candle fetch / network).
_STATUSES = ["win", "loss", "expired"]


def _make_decision(event_risk: str) -> dict:
    """A BUY decision whose only varying dimension is the event entry.
    ...
    fully determined by the ``evt:`` tag — trades sharing an Event_Risk bucket
    aggregate into the same group. The ``__unavailable__`` sentinel records an
    explicitly-unavailable event entry, exercising the ``evt:unknown`` bucket
    without fabricating a risk.
    """
    if event_risk == "__unavailable__":
        event_entry = {"available": False, "reason": "no event source configured"}
    else:
        event_entry = {
            "available": True,
    ...
    return {
        ...
        "stop_loss": 99.0,
        "take_profit": 103.0,
        "defensibility": {"event": event_entry},
    }


@st.composite
def _trade(draw):
    """A single already-scored trade spec: (event_risk, status, r_multiple)."""
    event_risk = draw(st.sampled_from(_EVENT_RISKS))
    status = draw(st.sampled_from(_STATUSES))
    r_multiple = draw(
        st.floats(min_value=-10.0, max_value=10.0, allow_nan=False, allow_infinity=False)
    )
    return (event_risk, status, r_multiple)
...
# ─────────────────────────────────────────────────────────────────────────────
# Property 25: Per-event-risk aggregation reports correct win-rate and expectancy
# ─────────────────────────────────────────────────────────────────────────────

# Feature: earnings-event-risk-gate, Property 25: Per-event-risk aggregation reports correct win-rate and expectancy
@settings(max_examples=25, deadline=None, suppress_health_check=[HealthCheck.too_slow])
@given(trades=st.lists(_trade(), min_size=1, max_size=30))
def test_property_25_per_event_risk_aggregation(trades):
    """Validates: Requirements 10.4, 10.5

    Recording a set of scored trades with varying event-risk buckets and then
    aggregating via ``get_stats`` reports, per event-extended ``setup_key``, a
    win-rate equal to wins/(wins+losses), an expectancy equal to the mean
    ...
    # event-extended setup_key.
    expected: dict = {}
    for event_risk, status, r_multiple in trades:
        decision = _make_decision(event_risk)
