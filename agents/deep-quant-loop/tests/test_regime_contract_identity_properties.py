"""Property-based test for contract identity on conforming results/markers (tools.py, task 5.6).

Feature: regime-detection-gate

This Hypothesis property exercises ``validate_contract``'s ``get_market_regime``
branch:

  * Property 12 (3.5, 3.7) — ``validate_contract`` is the identity on conforming
    results and markers: for any generated conforming ``get_market_regime``
    Regime_Label, and for any Unavailable_Marker, ``validate_contract(
    "get_market_regime", result)`` returns that result unchanged.

A conforming Regime_Label carries the three categorical states drawn from their
fixed enums (``trend_state`` in {trending, ranging, transitional},
``volatility_state`` in {low, normal, high}, ``favorability`` in {favorable,
unfavorable, neutral}) plus a ``measures`` object whose every named measure
(directional_strength / choppiness / efficiency_ratio / atr_percentile /
bb_width) is a finite number or ``null``. An Unavailable_Marker carries
``{"unavailable": true, "reason": ...}`` and (per AD-4) omits the states.
...
# A finite number or null — exactly what each named Regime_Measure is allowed to
# be in a conforming label (R2.4, R2.6, R3.4). Bools are excluded because the
...
def _conforming_label(draw):
    """A conforming ``get_market_regime`` Regime_Label.

    Every categorical state is drawn from its fixed enum and the ``measures``
    object carries each named measure as a finite number or null, so the label
    satisfies the contract that ``validate_contract`` enforces.
    """
    measures = {
        field: draw(_measure_value) for field in _REGIME_MEASURE_FIELDS
    }
    label = {
        ...
        "candles_used": draw(st.integers(min_value=1, max_value=10_000)),
    }
    return label
...
def _unavailable_marker(draw):
    ...
    Per AD-4 the marker omits Trend_State / Volatility_State / Favorability; it
    is recognized as an honest non-fatal result and must pass through unchanged.
    """
    marker = {
        "unavailable": True,
        "reason": draw(
...
# Feature: regime-detection-gate, Property 12
@settings(max_examples=200, deadline=None)
@given(result=_conforming_result)
def test_property_12_validate_contract_is_identity_on_conforming(result):
    """Validates: Requirements 3.5, 3.7

    For any conforming Regime_Label or any Unavailable_Marker,
    ``validate_contract("get_market_regime", result)`` returns that result
    ...
    try:
        returned = validate_contract("get_market_regime", result)
