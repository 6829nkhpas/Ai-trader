# start_website.ps1 — Start the full StratAI system for local development.
#
# The web app is now the only frontend target (the Tauri desktop shell was
# retired), so this is THE launcher — `start_system.ps1`'s desktop counterpart no
# longer applies.
#
#   • Frontend runs `npm run dev` (Next.js dev server, SSR + API routes).
#   • The route handlers under src/app/api/ proxy /kite, /questdb, /deepquant,
#     /tools and /sentiment to the local backend services via _gateway.ts.
#   • Same infrastructure (Redis, QuestDB, Redpanda) and backend services.

# ── PATH setup ────────────────────────────────────────────────────────────────
$env:PATH = "$env:USERPROFILE\.cargo\bin;C:\Program Files\CMake\bin;" + $env:PATH

# ── Force vibrant ANSI colors ─────────────────────────────────────────────────
...
# Store process objects to kill on exit
$script:processes = @()
...
function Cleanup {
    Write-Host "`nShutting down website system..." -ForegroundColor Yellow
    ...
    Write-Host "Website system shutdown complete." -ForegroundColor Green
}

# ── Main ──────────────────────────────────────────────────────────────────────
try {
    # Ports: 3000=Next.js, 8080-8083=WS agents, 8084=Tool Server,
    #        8086=Python Agent, 8087=Kite REST API,
    #        9000/9009=QuestDB, 5432=PG, 6379=Redis, 19092=Kafka
    Write-Host "==> Cleaning up stale processes and ports..." -ForegroundColor Magenta

    # Only kill application-level ports, never Docker-managed ones (5432, 6379, 9000, 9009, 19092)
    $portsToKill = @(3000, 8080, 8081, 8082, 8083, 8084, 8086, 8087)
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "Please start Docker Desktop and ensure the daemon is running before launching." -ForegroundColor Yellow
        return
    }
    ...
    # ── Start backend services (producers first, then consumers) ──────────────
    # Order: ingestion -> technical -> sentiment -> aggregator -> predictive -> quant-rag -> deep-quant-loop
    ...
    # `npm run dev` starts the Next.js dev server with SSR + API route handlers.
    # The handlers under src/app/api/ proxy /kite, /questdb, /deepquant, /tools
    # and /sentiment to the local backend services via _gateway.ts.
    Write-Host "Starting Next.js Frontend (Web / Browser mode on http://localhost:3000)..." -ForegroundColor Cyan
    Push-Location frontend
    $script:processes += Start-Process -NoNewWindow -PassThru -FilePath "cmd.exe" -ArgumentList "/c npm run dev"
    Pop-Location

    Wait-ForPort -Port 3000 -TimeoutSec 60 -Label "Next.js (:3000)"

    Write-Host ""
    Write-Host "============================================================" -ForegroundColor Green
    Write-Host "  StratAI Website is running!" -ForegroundColor Green
