...
- [x] 1. Parameter resolution foundation in `order_flow.py`
  - [x] 1.1 Create `order_flow.py` with `OrderFlowConfig` and `resolve_order_flow_config`
  ...
  - [x] 1.3 Write property test for pressure-threshold ordering enforcement
...
- [x] 2. Candle-derived proxy measure functions in `order_flow.py`
  - [x] 2.1 Implement the pure, candle-only proxy measure functions
  ...
  - [x] 2.2 Write property test for present, finite-or-null, correct proxy measures
    - **Property 3: Proxy measures are present, finite-or-null, and correct**
    - **Validates: Requirements 1.2, 1.3, 1.4, 4.3**

  - [x] 2.3 Write property test for bounded-measure clamping
    - **Property 4: Bounded measures are clamped within their range**
    - **Validates: Requirements 4.4**

- [x] 3. Tick-rule `compute_tick_ofi` in `order_flow.py`
  - [x] 3.1 Implement `compute_tick_ofi` (mirrors the Rust implementation, AD-8)
  ...
  - [x] 3.2 Write property test for the normalized signed-volume imbalance within bounds
    - **Property 6: Tick_OFI is the normalized signed-volume imbalance within bounds**
    - **Validates: Requirements 2.1, 2.4**

  - [x] 3.3 Write property test for Lee-Ready quote-location sign refinement
    - **Property 7: Quote location refines the tick sign (Lee-Ready)**
    - **Validates: Requirements 2.2**

  - [x] 3.4 Write property test for insufficient/degenerate ticks yielding unavailable (never fabricated neutral)
  ...
  - [x] 3.5 Write unit test that Python `compute_tick_ofi` mirrors the Rust `compute_order_flow_imbalance`
...
- [x] 4. Classification functions and `classify_order_flow` in `order_flow.py`
  - [x] 4.1 Implement classification and the top-level entry point
  ...
  - [x] 4.2 Write property test for classification determinism
    - **Property 1: Classification is deterministic**
    - **Validates: Requirements 1.6, 2.5**

  - [x] 4.3 Write property test for calculator purity (no input mutation, no network)
    - **Property 2: Calculator functions are pure (no input mutation)**
    - **Validates: Requirements 1.1, 1.7, 2.5**

  - [x] 4.4 Write property test for zero-denominator null and all-null-with-no-tick unavailability
...
- [ ] 6. `get_order_flow` tool and contract in `tools.py`
  - [x] 6.1 Extend `validate_contract` with a `get_order_flow` branch
  ...
  - [x] 6.2 Implement the `get_order_flow` tool and the live-ticks read
  ...
  - [x] 6.4 Write unit test that the tool consults candles and the live-ticks source only
