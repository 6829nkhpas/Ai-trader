...
- [x] 1. Threshold resolution foundation in `regime.py`
  - [x] 1.1 Create `regime.py` with `RegimeConfig` and `resolve_regime_config`
  ...
  - [x] 1.2 Write property test for per-threshold default fallback
    - **Property 28: Each threshold falls back to its documented default**
    - **Validates: Requirements 11.1, 11.2, 11.3, 11.4**

  - [x] 1.3 Write property test for volatility-percentile ordering enforcement
...
- [x] 2. Regime measure functions in `regime.py`
  - [x] 2.1 Implement the pure measure functions
...
- [x] 3. Classification functions and `classify_regime` in `regime.py`
...
- [x] 5. `get_market_regime` tool and contract in `tools.py`
  ...
  - [x] 5.2 Implement the `get_market_regime` tool
...
- [x] 7. Graph wiring of the tool in `graph.py`
  ...
  - [x] 7.2 Write unit tests for tool registration
...
- [x] 8. Regime in the defensibility record (`graph.py`)
  - [x] 8.1 Add the regime entry to `build_defensibility_record`
  ...
  - [x] 8.3 Write property test that absent regime is recorded as unavailable
    - **Property 19: Absent regime is recorded as unavailable**
    - **Validates: Requirements 7.3**

  - [x] 8.4 Write property test for the unfavorable-directional opposition statement
    - **Property 20: An unfavorable directional trade records the opposition statement**
    - **Validates: Requirements 7.4**

  - [x] 8.5 Write property test that the gate never modifies or blocks a decision
    - **Property 32: The regime gate never modifies or blocks a committed decision**
    - **Validates: Requirements 12.5, 12.6**

- [x] 9. Prompt integration of the Regime Gate (`graph.py`)
...
- [x] 10. Regime verification step in `stream_events.py`
  - [x] 10.1 Emit the regime `VERIFICATION_STEP`
  ...
  - [x] 10.2 Write property test for the regime verification step outcome mapping
...
- [x] 11. Journal setup-fingerprint extension in `journal.py`
  - [x] 11.1 Add the regime tag to `derive_setup_tags`
  ...
  - [x] 11.3 Write property test for per-regime aggregation metrics
...
- [x] 13. Backtest with-gate / without-gate comparison in `backtest.py`
  ...
  - [x] 13.2 Implement the gate logic and `compare` entry point
  ...
  - [x] 13.3 Write unit test that the backtest reuses the classifier
  ...
  - [x] 13.5 Write property test for gate exclusion/retention behavior
...
- [x] 14. Integration and end-to-end wiring
  ...
  - [x] 14.2 Write end-to-end example test for a favorable FIND-mode run
