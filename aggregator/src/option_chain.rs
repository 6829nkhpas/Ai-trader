// option_chain.rs — Pure option-chain resolution math (Phase F1).
//
// Moved here from `frontend/src-tauri/src/services/option_chain.rs` when the
// desktop shell was retired. Nothing about the math changed — the file had no
// Tauri coupling at all (its only import is `chrono::NaiveDate`), and all seven
// property-test modules came with it. The aggregator is now the only consumer:
// it owns the NFO instrument cache and the Kite quote path, which is everything
// the selection needs, so `option_chain_selector.rs` drives these functions on a
// timer and pushes the result to the ingestion control port.
//
