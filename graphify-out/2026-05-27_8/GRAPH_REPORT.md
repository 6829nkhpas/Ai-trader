...
## Corpus Check
- 243 files · ~228,633 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2390 nodes · 4236 edges · 205 communities (186 shown, 19 thin omitted)
...
Cohesion: 0.17
Nodes (18): TrendBias, useMultiTimeframeTrend(), biasBarColor(), biasColor(), generateMockAnomaliesForSymbol(), getFallbackSentiment(), InsightCard(), InsightCardProps (+10 more)
...
Cohesion: 0.07
Nodes (33): Alpha Suite V2 — Production Setup Guide, Architecture Reference, CMake Not Found (Local Dev Only), code:bash (cp .env.example .env), code:block10 (ERROR: Kite WS auth failed — 403 Forbidden), code:block11 (ERROR: DeepSeek API failure: NVIDIA NIM request timeout), code:block12 (error: failed to run custom build command for `rdkafka-sys`), code:env (# ── Zerodha Kite Connect ──────────────────────────────────) (+25 more)
...
Cohesion: 0.14
Nodes (13): 1. VWEPR (Volume-Weighted Exponential Price Regression) Curvature, 2. Multi-Source Pipeline Fusion & Deduplication, Ai-trader, code:rust (// In quant/vwepr.rs — Fit solver using determinants), code:text (/), code:env (# ── database configuration ───────────────────────────────), 📂 Core Component Directory Map, 🧠 Deep Quant Analytical Foundation (+5 more)
...
Cohesion: 0.42
Nodes (8): aggregateToTimeframe(), calculateEMA(), calculateRSI(), computeTrend(), generateSyntheticOhlc(), TIMEFRAME_CONFIGS, TimeframeTrend, useMultiTimeframeTrend Hook
